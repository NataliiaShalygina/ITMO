...
# Переменные - виджеты текста (будет обновляться) и кнопки (будет переключать и менять статус в зависимости от полученных данных)
        self.task_display = task_display_widget
        self.next_button = next_button_widget

# Переменные для передачи методам, а также переменные индексов
...
# Методы передачи значений со списков (геометрия) из окон
    ...
    # Методы осуществления поиска (три: по индексу (окно алгебра), по тема + тип и по учебник + номер (геометрия))
 
    def find_tasks_thema_id(self, selected_theme_id):

        # Формирует, отправляет запрос, получает результат из БД
        all_data = self.db.get_data_by_thema_id(selected_theme_id) 

	# Если значение не пустое - текст передаётся окну + фиксируются индексные значения, иначе выдаёт предупреждение
        if all_data:
            self.current_tasks = [row[0] for row in all_data]
            self.current_task_index = 0
            self._display_current_task()

        # Если индекс задач больше одной, активирует кнопку "Следующая", иначе - деактивирует (на случай повторного запроса)
            if self.next_button:
    ...
    # Два следующих метода работают аналогично, разница только в количестве пеменных и передаче запроса в БД
   ...
   # Методы обновления окошка (вызывается методом поиска) + работа кнопки
